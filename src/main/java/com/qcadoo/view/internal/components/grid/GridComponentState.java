...
public final class GridComponentState extends AbstractComponentState implements GridComponent {
    ...
    public GridComponentState(final DataDefinition dataDefinition, final GridComponentPattern pattern) {
        ...
        registerEvent("softDelete", eventPerformer, "softDeleteSelectedEntity");
    }
    ...
    protected void initializeContent(final JSONObject json) throws JSONException {
        ...
        if (json.has(JSON_SELECTED_ENTITIES) && !json.isNull(JSON_SELECTED_ENTITIES)) {
            ...
            for (int i = 0; Objects.nonNull(selectedEntitiesIds) && i < selectedEntitiesIds.length(); i++) {
    ...
    private void applyPredefinedFilter(final PredefinedFilter predefinedFilter) {
        if (Objects.isNull(predefinedFilter)) {
            return;
        }

        orderColumns.clear();

        if (Objects.nonNull(predefinedFilter.getOrderColumn())) {
            orderColumns
    ...
    private boolean gridIsEmpty() {
        return Objects.isNull(belongsToEntityId);
    }
    ...
    public void onScopeEntityIdChange(final Long scopeEntityId) {
        ...
        setEnabled(Objects.nonNull(scopeEntityId) && securityRolesService.canAccess(authorizationRole));
    }
    ...
    protected JSONObject renderContent() throws JSONException {
        ...
        if (Objects.nonNull(rowStyleResolver)) {
        ...
        if (Objects.nonNull(criteriaModifierParameter)) {
    ...
    public List<Entity> getSelectedEntities() {
        if (Objects.isNull(selectedEntities) || selectedEntities.isEmpty()) {
    ...
    public void setSelectedEntitiesIds(final Set<Long> selectedEntities) {
        ...
        if (Objects.isNull(selectedEntities) || selectedEntities.size() < 2) {
            multiselectMode = false;
        } else {
    ...
    protected class GridEventPerformer {
        ...
        // SOFT DELETE GRID
        public void softDeleteSelectedEntity(final String[] args) {
            if (selectedEntities == null || selectedEntities.isEmpty()) {
                return; // không có entity nào để xóa mềm
            }

            EntityOpResult result = EntityOpResult.successfull();

            // Nếu là weakRelation, chỉ cập nhật thuộc tính deleted 
            if (weakRelation) {
                for (Long selectedId : selectedEntities) {
                    Entity entity = getDataDefinition().getMasterModelEntity(selectedId);
                    entity.setField("deleted", true);
                    entity.setField("deletedDate", new Date());
                    entity.getDataDefinition().save(entity);

                    copyFieldValidationMessages(entity);
                }
            } else {
                // Nếu không phải weakRelation, cũng cập nhật deleted
                for (Long selectedId : selectedEntities) {
                    Entity entity = getDataDefinition().getMasterModelEntity(selectedId);
                    entity.setField("deleted", true);
                    entity.setField("deletedDate", new Date());
            ...
            if (selectedEntities.size() == 1) {
                ...
                addTranslatedMessage(
                    translateMessage("deleteMessages", String.valueOf(selectedEntities.size())),
                    MessageType.SUCCESS
                );
            }
        ...
        private void reload() {
            if (Objects.isNull(belongsToFieldDefinition) || Objects.nonNull(belongsToEntityId)) {
                ...
                if (Objects.nonNull(belongsToFieldDefinition) && !useDto) {
                ...
                try {
                    ...
                    if (Objects.nonNull(customRestriction)) {
                    ...
                    // deleted filter
                    try {
                        if (getDataDefinition() != null && getDataDefinition().getField("deleted") != null) {
                            criteria.add(SearchRestrictions.eq("deleted", false));
                        }
                    } catch (Exception ex) {
                        // Không làm gì cả nếu model không có field "deleted"
                        // hoặc DataDefinition chưa sẵn sàng
                    }
    ...
    @Override
    public void setFilters(final Map<String, String> newFilters) {
